...
public:
    ...
    action_name_(name)
  {
    ...
    //realtime_pub.reset(new realtime_tools::RealtimePublisher<std_msgs::String>(nh_, "/ur_driver/URScript",100));
    //realtime_pub = new
    //realtime_tools::RealtimePublisher<std_msgs::String> (nh_, "/ur_driver/URScript", 100,false);
...
void tool_vel_subCB(const geometry_msgs::TwistStampedConstPtr &msg ){
  ...
  void executeCB(const optimation_msgs::TrajectoryExecutionGoalConstPtr &goal)
  {
    ...
    //realtime_pub = nh_.advertise<std_msgs::String>("/ur_driver/URScript", 100);
  ...
  std::vector<double> calc_correction_vel(double goal, double speed ,double pos,double freq) {
    ...
    double time= 1/freq;
    double dist=goal-(pos);
    dist=std::abs(dist);

    correct[1]=(dist-(std::abs(speed)*time))*2/(time*time);
    correct[0]=(correct[1]*time)+std::abs(speed);
    std::cout<<"\n"<<"goalpose: "<<goal<<"\n";
    std::cout<<"\n"<<"dist: "<<dist<<"\n";
    std::cout<<"\n"<<"time: "<<time<<"\n";
  ...
  bool drive_to_goal_pose(const optimation_msgs::TrajectoryExecutionGoalConstPtr &goal){
    ...
    realtime_tools::RealtimePublisher<std_msgs::String> realtime_pub(nh_, "/ur_driver/URScript",1000,false);
...
*/
      ...
      //if (cur_move_time-cur_hz_diff.toSec()>=1/freq){
          ...
          if (i==vel.size()-2){
            ...
            movej_msg.data=movej_msg.data+"0.2,0.01,0,0)";
            ...
            effort_sum=effort_sum+((std::abs(effort[0])+std::abs(effort[1])+std::abs(effort[2])+std::abs(effort[3])+std::abs(effort[4])+std::abs(effort[5]))*48*(1/freq));
            ...
            */
            ...
            movej_msg.data="speedl([0,"+vel_str+",0,0,0,0],"+acc_str+","+tostr((1/freq)*10)+")";
            //*/
            //}
...
//std::this_thread::sleep_for(std::chrono::microseconds((1/freq)*1000));
    rate.sleep();
    //if (i<=vel.size()){
    //}
  ...
  bool set_start_pose(geometry_msgs::Pose goal_pose){
    std_msgs::String movej_msg;
    movej_msg.data=movel_URSstring(goal_pose);
    movej_msg.data=movej_msg.data+"0.8,0.3,4,0)";
    urscript_pub.publish(movej_msg);
    std_msgs::Float32 energy_msg;
    double freq=25;
    double joint_sum;
