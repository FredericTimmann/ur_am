...
double start_x=0.3;
double start_y=-0.00;
double start_z=0.082;
double translation_x =start_x;
double translation_y =start_y-0.25;
double translation_z =0.0788;
double RX_rot=2.4503;
double RY_rot=-2.412;
double RZ_rot=2.46;
double tcp_acc=0.75;
...
void get_next_Parameter_ext(printing_scene& scn){
  ...
  scn.last_Point=scn.next_Point;
   ...
   scn.next_Point.position.x=(scn.next_Point.position.x/1000)+translation_x;
   scn.next_Point.position.y=(scn.next_Point.position.y/1000)+translation_y;
   scn.next_Point.position.z=(scn.next_Point.position.z/1000)+translation_z;
   return;
 }
  ...
  switch (prstp) {
    ...
    if (tcp_distance==0){
      ...
      while (!time_reached){
        ...
        if (cur_move_time>=move_time*1.5){
...
//TODO temp aus und Robi wegfahren in safe position
  ...
  bool URScript_movel(bool extr, geometry_msgs::Pose goal_pose,double time,double tcp_speed,ros::Publisher urscript_pub,ros::Publisher goalpub,ros::Publisher pathpub)
  {
    ...
    if (tcp_speed<=0){
      tcp_speed=default_tool_velocity;
    }

    //double rate=((time*1.0));
    if (time<=0){
        time=0.1;
      }
      double x1;
